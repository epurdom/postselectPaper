...
#' @param file_naming_utils List. Paths and prefixes for loading/saving (see target function).
#'
#' @return A list as returned by the underlying simulation function (e.g. \code{used_sce} and \code{pa_de} for augData).
#'
#' @export
create_simulated_data <- function(sim_type, sim_prefix, new_id_check, num_de_genes, file_naming_utils) {
  if (sim_type == "mSim"){
    print("Creating simulated data for mSim")
    stop("Not implemented yet")
    # sim_data <- create_full_mSim_data(new_id_check = new_id_check, num_de_genes = num_de_genes, file_naming_utils = file_naming_utils)
  } else if (sim_type == "augData"){
    sim_data <- create_full_augmented_data_w_num_sim_genes(sim_prefix = sim_prefix, new_id_check = new_id_check, num_sim_genes = num_de_genes, file_naming_utils = file_naming_utils)
  } else {
...
#' clusterings under \code{file_naming_utils$sims_info_dir}.
#'
...
#' @param file_naming_utils List. Must contain \code{base_seurat_fn},
#'   \code{sims_info_dir}, \code{param_file_prefix}, and save prefixes:
#'   \code{de_args_save_prfx}, \code{condition_save_prfx},
#'   \code{sample_ids_save_prfx}, \code{gt_clusterings_save_prfx}.
...
#' @export
create_full_augmented_data_w_num_sim_genes <- function(
    sim_prefix, new_id_check, num_sim_genes, file_naming_utils, verbose = TRUE, save_down = TRUE) {
  print("Loading seurat null genes with leiden clusters")
  print(paste0("Base seurat file: ", file_naming_utils$base_seurat_fn))
  base_seurat <- readRDS(paste0(file_naming_utils$base_seurat_fn))
  neg_de_args <- tibble::tibble(name = rownames(base_seurat), is_simulated = FALSE)
  neg_de_counts <- base_seurat@assays$RNA@layers$counts

  print(paste0("Sim info file: ", file_naming_utils$sims_info_dir, "/", file_naming_utils$param_file_prefix, sim_prefix, ".Rds"))
  pa <- readRDS(paste0(file_naming_utils$sims_info_dir, "/", file_naming_utils$param_file_prefix, sim_prefix, ".Rds"))
  pa$n_de_genes <- num_sim_genes

  print("Preparing augmented data - randomizing sample/cell selections")
  start_time <- Sys.time()
  pa <- prep_aug_sim(pa, pa$config, processed_sim = base_seurat, verbose = verbose)
  end_time <- Sys.time()
  ...
  start_time <- Sys.time()
  augmented_data <- create_augmented_data(pa, pa$config, verbose = verbose)
  end_time <- Sys.time()
  counts_aug_data <- augmented_data$new_counts
  gt_clusterings <- augmented_data$kmeans_clusterings
  ...
  if(save_down){
    de_args$is_de_cell <- NULL # for space purposes, drop the is_de_cell entry for de_args
    de_args_fn <- paste0(file_naming_utils$sims_info_dir, "/", file_naming_utils$de_args_save_prfx, new_id_check, ".Rds")
    print(paste0("Saving de_args for simulation ", new_id_check, " to ", de_args_fn))
    saveRDS(de_args, de_args_fn)

    fake_condition_fn <- paste0(file_naming_utils$sims_info_dir, "/", file_naming_utils$condition_save_prfx, new_id_check, ".Rds")
    ...
    sample_ids_fn <- paste0(file_naming_utils$sims_info_dir, "/", file_naming_utils$sample_ids_save_prfx, new_id_check, ".Rds")
    print(paste0("Saving sample ids for simulation ", new_id_check, " to ", sample_ids_fn))
    saveRDS(used_sce$sample, sample_ids_fn)

    gt_clusterings_fn <- paste0(file_naming_utils$sims_info_dir, "/", file_naming_utils$gt_clusterings_save_prfx, new_id_check, ".Rds")
...
#' @param num_de_genes Integer. Number of DE genes.
#' @param file_naming_utils List. Path configuration (unused; mSim not implemented).
#'
#' @return Nothing; stops with an error.
#'
#' @export
create_full_mSim_data <- function(sim_prefix, id_check, lfc, num_de_genes, file_naming_utils) {
